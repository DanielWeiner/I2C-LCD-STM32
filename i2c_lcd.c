...
void lcd_send_cmd(I2C_LCD_HandleTypeDef *lcd, char cmd) {
	...
	data_t[0] = upper_nibble | 0x04 | lcd->backlight;  // en=1, rs=0
	data_t[1] = upper_nibble | 0x00 | lcd->backlight;  // en=0, rs=0
	data_t[2] = lower_nibble | 0x04 | lcd->backlight;  // en=1, rs=0
	data_t[3] = lower_nibble | 0x00 | lcd->backlight;  // en=0, rs=0
...
void lcd_send_data(I2C_LCD_HandleTypeDef *lcd, char data) {
	...
	data_t[0] = upper_nibble | 0x05 | lcd->backlight;  // en=1, rs=1
	data_t[1] = upper_nibble | 0x01 | lcd->backlight;  // en=0, rs=1
	data_t[2] = lower_nibble | 0x05 | lcd->backlight;  // en=1, rs=1
	data_t[3] = lower_nibble | 0x01 | lcd->backlight;  // en=0, rs=1
...
void lcd_gotoxy(I2C_LCD_HandleTypeDef *lcd, int col, int row) {
	...
	switch (row) {
		...
		case 3:
			...
			break;	// Fourth row
		default:
			return;	 // Ignore invalid row numbers
	}
...
void lcd_init(I2C_LCD_HandleTypeDef *lcd) {
	lcd->backlight = LCD_NOBACKLIGHT;  // Set backlight off
